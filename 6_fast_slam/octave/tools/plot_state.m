function plot_state(particles, landmarks, timestep, z, window)
    ...
    % SELECT PARTICLE WHICH IS CLOSEST TO MEAN VALUE
    % LOOKS LIKE WORKING. BUT ASSUMING THAT EACH PARTICLES' MAPS ARE OVERLAPPED
    %total_w = sum([particles.weight]);
    %mean_w = ([particles.pose] * [particles.weight]') / total_w;
    %err = [particles.pose] - repmat(mean_w, 1, 100);
    %ls_err = err(1,:).*err(1,:) + err(2,:).*err(2,:);
    %[bestWeight, bestParticleIdx] = min(ls_err);
